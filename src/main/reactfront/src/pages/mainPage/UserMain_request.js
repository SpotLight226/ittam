...
function UserMain_request() {
  ...
  const count_using = myRequestList.filter(
    (a) => a.userq_yn.includes('관리자사용') || a.userq_yn.includes('사원사용')
  ).length;
  const count_buy = myRequestList.filter(
    (a) =>
      a.userq_yn.includes('구매') &&
      (a.userq_yn.includes('반려') || a.userq_yn.includes('최종구매승인'))
  ).length;
  ...
  return (
    <main id="main" className="main">
      {openReqDetailModal && (
        <ReqDetailModal
          setOpenReqDetailModal={setOpenReqDetailModal}
          username={username}
              ...
              >
                ...
                <tbody>
                  {myRequestList
                    .filter(
                      (a) =>
                        a.userq_yn.includes('관리자사용승인') ||
                        a.userq_yn.includes('사원사용')
                    )
                    .map((a, i) => {
                      return (
                        <tr key={i}>
                          ...
                          <td>
                            <Link
                              to="#"
                              onClick={() => {
                                setOpenReqDetailModal(true);
                                setUserq_num(a.userq_num);
                              }}
                            >
                              {a.userq_title}
                            </Link>
                          ...
                          <td style={{ color: 'blue', fontWeight: '600' }}>
                            {a.userq_yn.includes('최종사용승인')
                              ? '승인'
                              : a.userq_yn.includes('사원사용') ||
                                a.userq_yn.includes('관리자사용승인')
                              ? '승인대기'
                              : a.userq_yn.includes('반려')
                              ? '반려'
                              : '승인대기'}
                          </td>
                        </tr>
                ...
                <tbody>
                  {myRequestList
                    .filter(
                      (a) =>
                        a.userq_yn.includes('구매') &&
                        (a.userq_yn.includes('사원구매') ||
                          a.userq_yn.includes('관리자구매승인'))
                    )
                    .map((a, i) => {
                      return (
                        <tr key={i}>
